...
public class GoogleMapFragment extends Fragment implements OnMapReadyCallback {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup viewGroup, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.drawer_layout, viewGroup, false);
...
//        change the file to parking spots because that is what we are supposed to show with markers
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(this.getContext().getAssets().open(dataset)))) {
...
//        skips first line
            ...
            try {
                while (count <= 15) // Reads first 15 coordinates to load data quickly, change this later to search coordinates within 500 m os searches location
                {
                    ...
                    LatLng address = getLocationFromAddress(this.getContext(), line);
